...
io.on('connection', socket => {
  ...
  socket.on('live:host-transfer', async p=>{try{const actor=await socketUser(socket,p?.token),roomId=cleanText(p?.roomId,40),targetSocketId=cleanText(p?.targetSocketId,80),live=liveByRoom.get(roomId);if(!actor||!live||live.hostUserId!==actor.id)return;const next=live.broadcasters.get(targetSocketId),prev=live.broadcasters.get(live.hostSocketId);if(!next||next.role!=='guest'||!prev)return;prev.role='guest';next.role='host';live.hostSocketId=targetSocketId;live.hostUserId=next.userId;live.hostDisplayName=next.displayName;io.to(`live:${roomId}`).emit('live:host-transferred',{roomId,hostSocketId:targetSocketId});io.to(`room:${roomId}`).emit('live:status',livePublic(live));io.to(`live:${roomId}`).emit('live:status',livePublic(live));await logModeration(actor.id,'live_host_transfer','user',next.userId,roomId,{sessionId:live.sessionId})}catch(e){logError(e,'live:host-transfer')}});
